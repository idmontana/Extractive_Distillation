...
parameter
yr(Net) "1 indica que en la etapa si hay reflujo";
yr(Net)=0;
yr('2')=1;

binary variables
yb(Net) "1 indica que en la etapa si hay boil up"
par(Net) "1 indica que la etapa es real fisicamente"
yfG(Net) "1 indica que en la etapa hay alimentacion de Glicerol"
yfAz(Net) "1 indica que en la etapa hay alimentacion de Mezcla Az."
;
...
*EqDPS(N,j,Net)$(ord(Net)>1 and ord(Net)<card(Net)).. (DPS(N,j,Net)*A0*(2*sqr(K0)))*101325=e=((1)*( (((sqr(V(N,j,Net)/(rhoV(N,j,Net))))/3600) )*((rhoV(N,j,Net))*sum(comp,MW(comp)*y(N,j,comp,Net)/100)/1000)*(1-sqr(poro))));
DPS.l(N,j,net)=1e-4;
...
*DownfCns(N,j,Net)$(ord(Net)>1 and ord(Net)<card(Net))..downF(N,j,Net) =l= 0;
*$Ontext
...
*FloodLCns(N,j,Net)$(ord(Net)>1 and ord(Net)<card(Net))..EntrainFloodLVar(N,j,Net)*par(Net) =l= 0;
*FloodVCns(N,j,Net)$(ord(Net)>1 and ord(Net)<card(Net))..EntrainFloodVVar(N,j,Net)*par(Net) =l= 0;
...
*EqFP(N,j,Net)$(ord(Net) le (card(net)-1))..FP(N,j,Net)*V(N,j,Net)=e= (L(N,j,Net))*((sigma0/sigma_d)**0.2)*(((sum((comp,Net1),MW(comp)*y('1','1',comp,Net1)/100)/1000)/(sum((comp,Net1),MW(comp)*x('1','1',comp,Net1)/100)/1000))**1.5);
...
*tot_inf_def.. tot_inf=e=(cost_sta+cost_col)*anfact;
*-------------------------------------------------------------------------------
*Función objetivo
*-------------------------------------------------------------------------------
variables
...
Equation pareq(Net);
parEq(Net)..par(Net)$((ord(Net) ne card(Net)) and (ord(Net) ne 1))=e=(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yr(Net1)))+(yb(Net))-(sum(Net1$((ord(Net1) ge 2) and (ord(Net1) le ord(Net))),yb(Net1)));
par.l(Net)$(ord(Net)<17)=1;
par.l(Net)$(ord(Net)>17)=0;

*----------------------MODEL AND SOLUTION---------------------------------------
model Extractive_MINLP /all/;
*model DSDA /all/;
...
solve Extractive_MINLP using MINLP minimizing zobj;
execute_unload "dicopt_Conopt4_notr_solution.gdx"